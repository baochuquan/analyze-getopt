...
static int generate_output(struct getopt_control *ctl, char *argv[], int argc)
{
    ...
    while ((opt =
                (getopt_long_fp(argc, argv, ctl->optstr,
                                (const struct option *)ctl->long_options, &longindex))) != EOF)
    {
        if (opt == '?' || opt == ':')
    ...
    while (tokptr)
    {
        ...
        if (len > 0)
        {
            if (tokptr[len - 1] == ':')
            {
                if (tokptr[len - 2] == ':')
                {
                    // 如果子串的末尾两个字符均为 ':'，则表示为可选参数选项
                    tokptr[len - 2] = '\0';
                    arg_opt = optional_argument;
                }
                else
                {
                    // 如果子串的末尾只有一个字符为 ':'，则表示必选参数选项
                    tokptr[len - 1] = '\0';
